...
class DataGenerator(Sequence):
    ...
    def get_spec_size(self, s_len):
        return next_power_of_2(s_len)

    def __data_generation(self, list_IDs_temp):
        spec_size = self.get_spec_size(self.data_length)

        X = np.zeros((self.batch_size, spec_size // 2 ))
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            if len(file_list) == 0:
                ...
                audio_file_fft = np.zeros((spec_size // 2))
            else:
...
def main():
    ...
    train_generator = DataGenerator(path+'train_soundscapes/', train_idxs, train_labels, batch_size=batch_size, data_length=data_length, num_labels=num_labels)
    val_generator = DataGenerator(path+'train_soundscapes/', val_idxs, train_labels, batch_size=batch_size, data_length=data_length, num_labels=num_labels)
    test_generator = DataGenerator(path+'test_soundscapes/', test_idxs, test_data, batch_size=batch_size, data_length=data_length, num_labels=num_labels)


    model = Sequential()
    model.add(Conv1D(64, input_shape=(128, train_generator.get_spec_size(160000) // 128 //2,), kernel_size=5, strides=4, activation='relu'))
